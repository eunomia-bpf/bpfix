...
harness:
  name: bpfix-replay
  script: bpfix-bench/tools/validate_benchmark.py
...
```
python3 bpfix-bench/tools/validate_benchmark.py --replay bpfix-bench
```

`bpfix-bench/tools/validate_benchmark.py` must rerun build/load/capture for every listed
...
1. run `bpfix-bench/tools/validate_benchmark.py --replay bpfix-bench`, or require a passing
