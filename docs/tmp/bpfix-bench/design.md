# BPFix-Bench 设计：面向 LLM One-Shot eBPF 修复的挑战集
...
`bpfix-empirical/` 已经是 verifier 诊断 empirical corpus：输入是重放 verifier log，
输出是分类、定位和诊断质量。

`bpfix-bench/` 不是再做一个小 benchmark。它是一个 source-first、repair-first
的挑战集，用来回答：

> BPFix 的 proof-aware diagnostic 能不能让强 LLM 在真实 eBPF verifier reject
> 上一次生成正确修复？
...
BPFix diagnostic improves one-shot LLM repair for hard eBPF verifier
...
| C2 | BPFix diagnostics improve repair success. | Same cases, same model, same prompt budget. | bpfix-mode pass rate and absolute gain over raw. | partial on dev40 run: 23/40 = 57.5%, +35.0 pp over raw |
...
## 和 `bpfix-empirical` 的区别

| 项目 | `bpfix-empirical` | `bpfix-bench` |
| --- | --- | --- |
| 目标 | 诊断质量评测 | LLM 一次修复能力压力测试 |
| 输入 | verifier log + case metadata | 源码 + raw log 或源码 + BPFix diagnostic |
...
- raw 和 bpfix 两个模式使用同一模型、温度、token budget、timeout；
...
- pass/fail reason；
- model/config/commit；
- raw 与 bpfix prompt 的字符数，避免把“诊断更短”误报成纯 proof signal
  改进。
...
- BPFix diagnostic 应该暴露 raw log 难以稳定推断的 proof signal；
...
- 不用人工判断“看起来修好了”；
- 不把 `bpfix-empirical` 的 235 个样本重新采样成小集合。
...
| B3 | C2 | BPFix one-shot repair | source + BPFix diagnostic | pass rate, gain over raw | same `test.py` | must |
...
- `python3 bpfix-bench/tools/run_suite.py --smoke` 通过；
- `python3 bpfix-bench/tools/refresh_case_artifacts.py` 能重抓 raw/bpfix diagnostic；
- runner 能生成 raw/bpfix prompt；
- runner 能在有 llama.cpp server 时发起 one-shot 修复并用 oracle 判定。

paper 阶段：

- `clean60` 至少 60 个 admitted heldout cases；
- `python3 bpfix-bench/tools/audit_splits.py --split bpfix-bench/splits/clean60.txt --expected-count 60 --disallow-overlap bpfix-bench/splits/dev40.txt --audit-cases --smoke` 通过；
- Qwen27B raw-log one-shot pass rate < 30%；
- 同一模型 bpfix-mode pass rate 显著高于 raw；
...
- BPFix diagnostic：18/18 pass = 100.0%；
...
30%，bpfix-mode one-shot 高于接近 70% 的目标线。它也说明 BPFix diagnostic
...
- `python3 bpfix-bench/tools/audit_cases.py`：40/40 pass；
- `python3 bpfix-bench/tools/audit_cases.py --smoke`：40/40 pass；
- 40 个 case 都包含源码、raw verifier log、BPFix diagnostic 和可执行
  `test.py` oracle；
- 所有 `diagnostic.txt` 都是 supported diagnostic，没有 unknown。

当前 40-case corpus 的 BPFix diagnostic 分布：
...
BPFix diagnostic 带来 +35.0 percentage-point 的真实修复提升，但 57.5% 低于
...
真实 Qwen27B raw/bpfix repair run”。还不能说“BPFix repair assistance 已达到
