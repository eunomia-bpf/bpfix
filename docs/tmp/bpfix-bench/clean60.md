# BPFix-Bench Clean60 Benchmark Protocol

最后更新：2026-06-17

`clean60` 是 `bpfix-bench` 的论文主 benchmark 目标：60 个新的、无污染的、
...
- 从 `bpfix-empirical` 的 235 个 replay logs 里抽样；
...
```bash
python3 bpfix-bench/tools/audit_splits.py \
...
benchmark 运行。`run_suite.py --split bpfix-bench/splits/clean60.txt` 对空 split
...
`bpfix-empirical/cases/**/*.c` 都会进入比较。当前 fail 条件是 Jaccard >= 0.82 或
...
```bash
make bpfix-bench-real-seed-candidate-gate
```

正式论文结果还必须通过 paper gate，它把 split admission 和 prompt freeze 绑定起来：

```bash
make bpfix-bench-clean60-paper-gate \
  PROMPT_MANIFEST=bpfix-bench/splits/clean60.prompts.json
```
...
8. admission gate 会扫描本地 `bpfix-bench/results/**/summary.json`；任何已经出现在
...
- 每个 case 必须有 `challenge_flags` 对象：
  ...
  `true`，表示独立 reviewer 已检查该 case 不是对 dev40、bpfix-empirical 或同 split
  其他 case 的语义近重复。
...
```bash
python3 bpfix-bench/tools/prompt_manifest.py \
  --split bpfix-bench/splits/clean60.txt \
  --expected-count 60 \
  --output bpfix-bench/splits/clean60.prompts.json
```
...
python3 bpfix-bench/tools/prompt_manifest.py \
  --split bpfix-bench/splits/clean60.txt \
  --expected-count 60 \
  --verify bpfix-bench/splits/clean60.prompts.json
```
...
每个模型的一组 clean60 结果必须在报告前通过 `make bpfix-bench-result-gate`。
...
python3 bpfix-bench/tools/audit_results.py \
  --split bpfix-bench/splits/clean60.txt \
  --expected-count 60 \
  --prompt-manifest bpfix-bench/splits/clean60.prompts.json \
...
```bash
make bpfix-bench-result-gate RESULT_SUMMARIES='\
  /path/to/source-only/summary.json \
  /path/to/raw/summary.json \
  /path/to/trimmed-raw/summary.json \
  /path/to/bpfix/summary.json' \
