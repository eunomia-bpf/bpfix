# BPFix-Bench 40-Case Milestone Record
...
`bpfix-empirical/` 的 taxonomy 评测，也不使用 label agreement 当成功标准。
...
python3 bpfix-bench/tools/audit_cases.py --smoke
```

40 个 case 全部 admitted 后必须再跑：

```bash
python3 bpfix-bench/tools/run_suite.py --mode raw ...
python3 bpfix-bench/tools/run_suite.py --mode bpfix ...
```
