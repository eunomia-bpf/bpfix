...
C3 proof-family coverage & 常见 verifier failure 能映射到具体 error id 和 required proof & 156/235 known error id，91/235 有 related proof spans & partial \\
C4 taxonomy routing & 系统能区分 source bug、lowering artifact 和环境/配置失败 & coarse failure-class 与 taxonomy agreement 为 159/235；environment 和 limit recall 为 100\% & partial \\
C5 user advantage & 与 terminal-message baseline 相比，用户得到更多结构化定位信息 & label-proxy baseline 显示 root-pc exact 88/140；60-case blind audit 中 \sys root exact/near 为 18/26 和 21/27，correct help 为 24/60 和 23/60 & partial \\
\bottomrule
\end{tabularx}
\end{table*}
...
\begin{enumerate}
  ...
  \item \textbf{Diagnostic coverage：} 当前 error catalog 覆盖多少 case，哪些 case 仍是 \code{BPFIX-UNKNOWN}？
...
同时，覆盖仍不完整。79/235 case 仍输出 \code{BPFIX-UNKNOWN}，这说明 error catalog 和 proof-family recognizer 还没有覆盖所有 selftest/dynptr/kfunc/iterator 等现代 verifier 错误。Related proof spans 只有 91/235，说明 proof lifecycle reconstruction 已在一部分 case 上工作，但还不是所有 case 的默认能力。因此，当前论文不应声称“完整自动 root-cause localization”；更可信的 claim 是：\sys 建立了 log-first diagnostic architecture，并已在 235 个 replayable reject 上证明鲁棒性，在 156 个 case 上给出具体 error id，在 91 个 case 上给出 proof lifecycle span。

表~\ref{tab:taxonomy-confusion} 进一步给出 coarse routing 的真实状态。当前 runtime failure-class 与 benchmark taxonomy 在 159/235 个 case 上相同；去掉 4 个 low-confidence 标签后为 156/231，结论基本不变。这个数字不能被写成最终诊断准确率，因为 taxonomy 只有粗粒度类别，而最终诊断还必须评价 required proof、causal mechanism、root span 和 false claim。它的价值在于暴露实现缺口：environment/configuration 和 verifier limit 已能被当前 classifier 路由，但 false positive 仍全部被当作 source/lowering 类处理，scalar range 与 pointer provenance case 也会把 source bug 和 lowering artifact 混在一起。表~\ref{tab:per-source-coverage} 显示这种缺口在真实项目和 Stack Overflow case 上更明显，而 kernel selftest 的 terminal message 更规范，coarse agreement 更高。

\subsection{Baselines}

我们把 baseline 分成已运行的 label-proxy baseline 和正式 submission 前必须补的 human-scored baseline。已运行 baseline 是 terminal dictionary：只看 \code{case.yaml} 中的 terminal error，用一组 regex 输出 error id、failure class 和粗略 next action。它代表 final-message translator 的下界，不读取 full verifier trace、source annotation 或 object metadata。表~\ref{tab:proxy-baseline} 给出结果：terminal dictionary 在 coarse taxonomy agreement 上更高，因为 benchmark 多数类是 source bug，保守输出 source bug 已经能拿到高分；\sys 的增量则体现在 primary span、related proof spans 和 root-pc localization。这个结果迫使本文收紧 claim：当前证据支持“\sys 提供 terminal-only baseline 没有的结构化定位上下文”，尚不支持“\sys 在所有诊断正确性指标上优于 baseline”。
...
\midrule
known error id & 156/235 & 145/235 & \sys 多覆盖 dynptr、environment 和 limit pattern \\
error-id exact & 46/235 & 43/235 & label error id 比 runtime catalog 更细，二者都偏低 \\
taxonomy agreement & 159/235 & 199/235 & terminal baseline 因多数类为 source bug 而占优，不能证明 root-cause quality \\
taxonomy macro-F1 & 0.543 & 0.558 & coarse routing 仍不是 \sys 的强项 \\
lowering recall & 4/24 & 0/24 & terminal-only 无法识别 proof established then lost \\
environment recall & 11/11 & 11/11 & 显式 capability pattern 已覆盖当前 env cases \\
...
related proof spans & 91/235 & 0/235 & \sys 的主要增量是 proof lifecycle context \\
root pc exact, labeled n=140 & 88/140 & 0/140 & span pc 候选命中 label root instruction \\
root pc within 5, labeled n=140 & 106/140 & 0/140 & coarse localization 已可度量，但不是完整 source-span correctness \\
next-action proxy exact & 119/235 & 124/235 & 自动 action proxy 噪声较大，需 expert/user study 确认 help quality \\
\bottomrule
\end{tabularx}
\end{table*}
