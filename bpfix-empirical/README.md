...
`bpfix-empirical` is the replayable verifier-failure corpus used by BPFix. The only
...
Run the validator before treating this checkout as a valid local empirical corpus:
...
The empirical corpus is environment-sensitive. A case can be valid on one kernel,
...
`manifest.yaml` defines the fixed per-case defaults for this frozen empirical corpus:
...
`vendor/libbpf` submodule through `bpfix-empirical/libbpf.mk`, so replay does not
...
Run BPFix over every admitted replay log in the empirical corpus:
