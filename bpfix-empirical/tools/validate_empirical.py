#!/usr/bin/env python3
"""Validate and replay a bpfix empirical corpus directory."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--replay", type=Path, required=True, help="Empirical corpus root, e.g. bpfix-empirical")
    parser.add_argument("--timeout-sec", type=int, default=60, help="Per-command timeout in seconds")
    args = parser.parse_args(argv)

    report = validate_empirical(args.replay.resolve(), args.timeout_sec)
...
def validate_empirical(empirical_root: Path, timeout_sec: int) -> dict[str, Any]:
    report: dict[str, Any] = {
        "valid": False,
    ...
    manifest_path = empirical_root / "manifest.yaml"
    if not empirical_root.exists():
        report["errors"].append(f"empirical corpus root does not exist: {empirical_root}")
        return report
    ...
    try:
        legacy_rejected_indices = load_legacy_rejected_indices(empirical_root)
    except Exception as exc:  # noqa: BLE001
    ...
    for entry in case_entries:
        case_report = validate_case(empirical_root, manifest, entry, timeout_sec, legacy_rejected_indices)
...
def validate_case(
    ...
    case_dir = empirical_root / path_value if isinstance(path_value, str) else empirical_root / "cases" / case_id
...
def validate_case_metadata(
    ...
    if "split" in entry:
        errors.append("manifest case must not define split; bpfix-empirical has a single case set")
    if "split" in reporting:
        errors.append("case.reporting must not define split; bpfix-empirical has a single case set")
    if "quarantine" in reporting:
        errors.append("case.reporting must not define quarantine; keep non-primary cases outside bpfix-empirical")
...
def load_legacy_rejected_indices(empirical_root: Path) -> dict[str, int]:
    path = empirical_root / "raw" / "legacy-insn-numbering.yaml"
    if not path.exists():
        return {}
