...
only under `bpfix-empirical/cases/`.
...
- `kernel_selftests/*.yaml`: kernel selftest verifier-log fixtures retained from the same unified empirical root.
...
source:
  ...
  title: ...
collector:
  original_path: bpfix-empirical/raw/so/stackoverflow-70750259.yaml
content:
...
- `replay_valid`: admitted to `bpfix-empirical/cases/`.
...
- `out_of_scope_non_verifier`: collected record is not a verifier-reject
  empirical corpus candidate.

Policy:

- Raw inclusion means only “collected external evidence”, not empirical corpus
  admission.
