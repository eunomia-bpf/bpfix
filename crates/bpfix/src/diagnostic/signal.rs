...
const SIGNAL_INFOS: &[SignalInfo] = &[
    ...
    signal_info!(PointerShiftDropsProvenance, "lowering_artifact", "verifier-visible compiler lowering hides the required proof", "repair_hint", Provenance, "lowering_artifact_signal", "compiler-lowered integer operation drops pointer provenance", "Keep packet or context pointers in verifier-tracked 64-bit pointer values; avoid materializing them through 32-bit scalar arithmetic before the access.", 20, false, false, None, None, None),
    ...
    signal_info!(ContextAccessSourceArgumentMismatch, "source_bug", "tracing context argument does not match the verifier-visible context ABI", "repair_hint", Context, "verifier_state_signal", "verifier state shows a BPF_PROG argument load or tracepoint/raw-tracepoint context field produced a scalar where the source treats it as a typed pointer", "Use the program type's verifier-supported context layout: for fentry, use only BPF_PROG arguments whose BTF type is verifier-supported and avoid unsupported pointer type loads; for tracepoint/raw-tracepoint arguments, use the correct context field or helper-based user/kernel memory read before dereferencing.", 10, true, true, Some("use the verifier-supported context ABI for this program type before treating context arguments as typed pointers"), Some("context argument is scalar in verifier state but typed as a pointer in source"), Some("BPFIX-E011")),
