...
> log 时能否一次生成可工作的修复？把 raw log 换成 BPFix plain-text
...
pilot 的历史结果 raw `5/18` 和 bpfix diagnostic `18/18` 仍保留在
...
raw verifier log。`diagnostic.txt` 是同一份 log 的 BPFix plain-text 输出。
`test.py` 是唯一 oracle：候选修复必须通过它。

## Smoke Test

检查 case 文件结构、BPFix diagnostic、oracle prompt 是否泄露 `test.py` 信息：
...
重新抓取 raw log 和 BPFix diagnostic：
...
`bpfix` mode 的 prompt 会把 BPFix plain-text diagnostic 原样给模型：
source span 是 verifier 拒绝的操作，相关 note/help 是可用的 proof context
而不是 oracle 答案，required proof 和 help 是候选源码必须满足的约束。如果
