...
verifier-visible pointer, read `drop_proto` from the map value, and use that
field to decide whether to drop the packet. The oracle updates the pinned map
with a protocol value unknown to the prompt, requires a successful verifier
trace with a map lookup and a map-value load, and runs IPv4/non-IPv4 packet
tests.
