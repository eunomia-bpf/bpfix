...
reject_package_paths() {
    ...
    if printf '%s\n' "${manifest}" | grep -Eq '^(bpfix-bench|docs/tmp|vendor|docs/bpfix-py|docs/workshop-paper-plan\.md|docs/research-plan\.md)(/|$)'; then
        echo "${crate} package includes non-release project material:" >&2
        printf '%s\n' "${manifest}" |
            grep -E '^(bpfix-bench|docs/tmp|vendor|docs/bpfix-py|docs/workshop-paper-plan\.md|docs/research-plan\.md)(/|$)' >&2
        exit 1
    fi
}
