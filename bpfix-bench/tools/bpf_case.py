#!/usr/bin/env python3
"""Shared oracle helpers for bpfix-bench cases."""
...
def run_pinned(pin: Path, data: bytes) -> tuple[int, CommandResult]:
    with tempfile.NamedTemporaryFile(prefix="bpfix-bench-packet-", delete=False) as packet:
        packet.write(data)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a bpfix-bench case oracle.")
