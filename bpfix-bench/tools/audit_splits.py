#!/usr/bin/env python3
"""Audit bpfix-bench split files and heldout contamination rules."""
...
def upstream_root(root: Path) -> Path:
    return Path(os.environ.get("BPFIX_BENCH_UPSTREAM_ROOT", root.parent)).resolve()
...
def bpfix_bench_source_fingerprints(root: Path) -> dict[str, list[str]]:
    cases_root = root / "bpfix-empirical" / "cases"
...
def split_manifest_path(split: Path, root: Path) -> Path:
    splits_dir = root / "bpfix-bench" / "splits"
    known = {
...
def local_prior_result_cases(root: Path) -> dict[str, list[str]]:
    ...
    results_dir = root / "bpfix-bench" / "results"
    ...
    *,
    ...
    if repo is None:
        errors.append(
            ...
            "set BPFIX_BENCH_UPSTREAM_ROOT for paper-grade provenance checks"
        )
        return report
...
def audit_split(args: argparse.Namespace) -> dict[str, Any]:
    ...
    split_implies_dev40 = args.split.resolve() == (root / "bpfix-bench" / "splits" / "dev40.txt").resolve()
    ...
    if profile in {"candidate", "clean60"}:
        dev40_split = root / "bpfix-bench" / "splits" / "dev40.txt"
    ...
    if bpfix_bench_sources:
        ...
        if bpfix_bench_overlap:
            ...
            errors.append(f"buggy.bpf.c content overlap with bpfix-empirical: {formatted}")
    if profile in {"candidate", "clean60"} and not missing_cases and not split_duplicates:
        ...
        bench_matches = near_duplicate_source_matches(
            ...
            other_label="bpfix-empirical",
        )
        if bench_matches:
            near_duplicates["bpfix_bench"] = bench_matches
            errors.append(
                "near-duplicate buggy.bpf.c sources with bpfix-empirical: "
