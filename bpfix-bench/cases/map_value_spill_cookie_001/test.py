...
def non_ip_packet_with_icmp_protocol_offset() -> bytes:
    packet = bytearray(bytes.fromhex("00112233445566778899aabb0806") + (b"\x00" * 64))
    packet[23] = 1
    return bytes(packet)


def truncated_ipv4_packet_with_icmp_protocol_offset() -> bytes:
...
if __name__ == "__main__":
    raise SystemExit(
        run_case(
            ...
            functional_tests=[
                ("icmp_drops_from_spilled_default_when_override_missing", lambda: ipv4_packet(1), 1, [("configs", CONFIG_KEY0, map_value(1))]),
                (
                ...
                ("udp_drops_from_spilled_default_when_override_missing", lambda: ipv4_packet(17), 1, [("configs", CONFIG_KEY0, map_value(17))]),
                (
                ...
                (
                    ...
                    2,
                    [("configs", CONFIG_KEY0, map_value(1))],
                ),
                (
                    "truncated_ipv4_pass_even_with_icmp_offset_byte",
                    truncated_ipv4_packet_with_icmp_protocol_offset,
                    2,
                    [("configs", CONFIG_KEY0, map_value(1))],
                ),
            ],
