# Shared local libbpf build for bpfix-bench selftest loaders.
#
# Case Makefiles are executed from bpfix-bench/cases/<case_id>, so the default
# benchmark root is two directories up from the case directory.

BENCH_ROOT ?= $(abspath ../..)
REPO_ROOT ?= $(abspath $(BENCH_ROOT)/..)

LIBBPF_SRC ?= $(REPO_ROOT)/vendor/libbpf/src
LIBBPF_BUILD ?= $(REPO_ROOT)/target/bpfix-bench/libbpf-build
LIBBPF_INSTALL ?= $(REPO_ROOT)/target/bpfix-bench/libbpf-install
